...
const CATEGORY_HINTS = [
  ...
  {
    key: "reasoning",
    match: /Reasoning|Logical|Puzzle|Sequence|Analogy|Direction|Seating/i,
  },
  {
    key: "numerical",
    match: /Aptitude|Quant|Numerical|Math|Ratio|Interest|Probability|Percent/i,
  },
  {
    key: "advanced_qr",
    match:
      /Advanced Quant|Calculus|Integral|Derivative|Matrix|Vector|Bayes|Limit|Combinatorics/i,
  },
  {
    key: "advanced_coding",
    match: /Coding|Program|Algorithm|Function|Code|Pseudo/i,
  },
];
...
function inferCategory(stem, explicit) {
  ...
  if (/sort|array|string|linked list|tree|graph|stack|queue/.test(lower))
    return "advanced_coding";
  if (
    /integral|derivative|matrix|vector|limit|probability|bayes|gcd|lcm/.test(
      lower
    )
  )
    return "advanced_qr";
  if (
    /percent|ratio|interest|simple|compound|profit|loss|average|sum|difference|speed|distance|time/.test(
      lower
    )
  )
    return "numerical";
  if (
    /series|pattern|direction|seating|analogy|venn|puzzle|relation|syllogism|blood relation/.test(
      lower
    )
  )
    return "reasoning";
...
async function main() {
  ...
  console.log(
    "Counts per category:",
    Object.fromEntries(Object.entries(generated).map(([k, v]) => [k, v.length]))
  );
  const total = Object.values(generated).reduce((a, b) => a + b.length, 0);
  if (total === 0) {
    console.log("\nAll zero: heuristics failed to match. Try these tweaks:");
    console.log(
      "- Open one PDF, copy some question lines, and adjust QUESTION_REGEX."
    );
    console.log(
      "- Sometimes numbers are followed by multiple spaces or unusual punctuation."
    );
